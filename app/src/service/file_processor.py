...
async def download_file(source_blob_name, destination_file_name):
    ...
    logging.info(f"file downloading started {blob}")
...
async def unzip_with_native(zip_file, extract_to):
    logging.info(f"Unzipping file {zip_file} to {extract_to}")
...
async def zip_with_native(file_name, zip_file):
    logging.info(f"Zipping file {file_name} to {zip_file}")
...
async def apply_karaoke(folder_name):
    logging.info(f"Applying karaoke effect to folder name {folder_name}")
    ...
    logging.info(f"Running command: {spleeter_cmd}")
...
async def cleanup(file_name):
    logging.info(f"Cleaning up {file_name}...")
...
async def deduct_credits(file_id: str, user_id: str, duration: int, file_sha: str):
    
    logging.info(f"Deducting {duration} credits for user {user_id}")
    async with aiohttp.ClientSession() as session:
        async with session.post(credits_url, json={"fileId": file_id, "installationId": user_id, "duration": duration, "sha": file_sha}) as response:
            if response.status == 200:
                logging.info("Credits deducted successfully")
            else:
                logging.error(f"Failed to deduct credits: {response.status}")
...
async def process_file(source_file_name: str, jwt_token: str, device_token: str):
    ...
    await deduct_credits(source_file_name, header["installation_id"], header["duration"], header["file_sha"])

    logging.info(f"File {source_file_name} processed")
